<template>
  <div class="home">
    <div class="top">
      ...
      <button @click="onClickShow">查看</button>
    </div>

    <div class="content">
      <div>
        <p v-for="ol in openLive" :key="ol">{{ ol }}</p>
      </div>
      <div>
        <p v-for="st in sendText" :key="st">{{ st }}</p>
      </div>
    </div>
  </div>
</template>
...
async function onClickShow() {
  ...
  if (!vlaue.value) return;
  const roomData = await getRoomId(vlaue.value);
  if (roomData.code) {
    alert("房间号错误");
    return;
  }
  ...
  openSocket(socketData.data.host, roomData.data.room_id);
}

async function getRoomId(id) {
  const res = await Axios.get(`/api/room/v1/Room/room_init?id=${id}`);
  return res.data;
}

async function getWebSocketHost(roomid) {
  const res = await Axios.get(
    `/api/room/v1/Danmu/getConf?room_id=${roomid}&platform=pc&player=web`
  );
  return res.data;
}
      ...
      if (packet.op == 5) {
        ...
        for (let i = 0; i < packet.body.length; i++) {
          ...
          if (element.cmd == "DANMU_MSG") {
            sendText.value.push(
              `uid:${element.info[2][0]},用户:${element.info[2][1]},说:${element.info[1]}`
            );
            console.log(
              "uid: " +
                element.info[2][0] +
          ...
          //cmd = INTERACT_WORD 有人进入直播了
            ...
            openLive.value.push(`进入直播: ${element.data.uname}`);
